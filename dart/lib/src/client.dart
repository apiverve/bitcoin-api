...
class BitcoinValidationException implements Exception {
  final List<String> errors;

  BitcoinValidationException(this.errors);

  @override
  String toString() => 'BitcoinValidationException: ${errors.join("; ")}';
}
...
///
/// For more information, visit: https://apiverve.com/marketplace/bitcoin?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [currency] - The currency to get the price in
class BitcoinClient {
  final String apiKey;
  ...
  BitcoinClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/bitcoin',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [BitcoinValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw BitcoinValidationException(errors);
    }
  }

  /// Execute a request to the Bitcoin Price API.
  ///
  /// Parameters are validated before sending the request.
  Future<BitcoinResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw BitcoinException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return BitcoinResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw BitcoinException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw BitcoinException('Resource not found');
      } else {
        throw BitcoinException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is BitcoinException) rethrow;
      throw BitcoinException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Bitcoin Price API client.
class BitcoinException implements Exception {
  final String message;

  BitcoinException(this.message);

  @override
  String toString() => 'BitcoinException: $message';
}
